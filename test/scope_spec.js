...
describe("Scope", function () {
  ...
  describe("digest", function () {
    ...
    it("calls the listener function when the watched value changes", function () {
      ...
      scope.counter = 0;
      scope.$watch(
        function (aScope) { return aScope.someValue; },
        function (newValue, oldValue, aScope) { aScope.counter++; }
      );
    ...
    it("does not end digest so that new watches are not run", function() {
      ...
      scope.$watch(
        ...
        function (newValue, oldValue, aScope) {
          aScope.$watch(
            function (bScope) { return bScope.aValue; },
            function (newValue, oldValue, bScope) {
              bScope.counter++;
            }
          );
        }
    ...
    it('allows async $apply with $applyAsync', function (done) {
      ...
      expect(scope.counter).toBe(1);
      expect(scope.aValue).toBe(false);
      setTimeout(function () {
        expect(scope.counter).toBe(2);
        expect(scope.aValue).toBe('abc');
        done();
      }, 50);
    });
